...
void Movement::getScansBetween(double min, double max,
...
bool Movement::willCollide(std::vector<double> distances,
...
void Movement::uTurn(){
	...
	while (selfRotate > 0) {
		selfRotate--;
		if (selfRotate >= 40)
			_hamster->sendSpeed(-0.4, -45);
		else if (selfRotate < 40)
			_hamster->sendSpeed(0.4, 45);
		HamsterAPI::Log::i("Client", "U-Turn");
	}
}
void Movement::stopMoving() {
	_hamster->sendSpeed(0.0, 0.0);
}
